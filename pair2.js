...
const config = {
  ...
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/f10i6l.jpg' }
};
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> *𝙉𝘼𝙍𝙐𝙏𝙊 - 𝙈𝘿 -  𝙈𝙄𝙉𝙄*',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '© Naruto MD'; // dynamic bot name
